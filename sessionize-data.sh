...
for i in {1..100}; do
 ...
 docker-compose exec ksqldb-server curl -X PUT connect:8083/connectors/DATAGEN_CLICKSTREAM/pause

 echo "Pausing the clickstream datagen for 35 seconds"
 sleep 35


 docker-compose exec ksqldb-server curl -X PUT connect:8083/connectors/DATAGEN_CLICKSTREAM/resume
